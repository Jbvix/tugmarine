# Spec: validação de uso e admin (São Luís)
...
O autor precisa ver quem usou a calculadora, qual saldo saiu, se o chemaq marcou o resultado como correto, e poder responder-lhe no próprio app — sem email e sem WhatsApp.
...
1. No Command Center escolhe o **rebocador na lista oficial da frota** (não texto livre). Porto e nome do chemaq continuam livres.
...
Identidade da sessão: navio oficial + IMO/MMSI/classe apelido da frota + porto + chemaq + device id.
...
- Números: sessões totais, sessões hoje, navios distintos, chemaq distintos, % correto / incorreto.
- Lista filtrável por navio, classe apelido, veredito, data.
...
`verdict` só aceita `correct` | `incorrect`. `totalLiters` deve ser a soma dos tanques (servidor recalcula e usa a soma). Sem `vessel` da frota ou sem tanques → 400.
...
- `index` — array leve para lista/stats (id, datas, navio, chemaq, veredito, total, unread flags)

Volume esperado: dezenas de sessões na campanha. Se passar disso, migrar para Netlify Database sem mudar o contrato da API.
...
- Select do Command Center só lista nomes de `FLEET_VESSELS`.
